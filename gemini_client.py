...
# Revision: 26
# Description: Implements Step 2 of the development plan.
# Updates the ChatSession to handle the structured prompt from the
...
from turn import Turn, PromptType # <-- Import PromptType
...
# ... (Models class and GeminiClient class remain unchanged) ...
class Models:
    """Available Gemini model variants with their identifiers."""
    DEFAULT = "gemini-1.5-pro"  # Default high-capability model
    FLASH = "gemini-1.5-flash"  # Faster, more efficient model for rate-limit fallback
...
class ChatSession:
    ...
    async def send_message(self, prompt: PromptType) -> AsyncGenerator[Dict[str, Any], None]:
        """
        Send a message to the model and stream the response.

        Args:
            prompt: User message to send (string or structured list of parts)
